...
n=6
nnn=2^n;
fclose all; fhx=fopen('austriax.txt','r'); fhy=fopen('austriay.txt','r');
...
figure(2); clf; % Clear existing Figure 2
...
for i=0:m-1 %detalumo didinimo ciklas
    ...
    figure(2);subplot(m+1, 1, i+2, 'Position', [0.1, 0.1+(m-i-1)*0.8/m, 0.8, 0.8/m]), axis equal,axis([xmin xmax ymin ymax]),hold on,grid on, plot(hx,hy,'Linewidth',2);title(sprintf('lygyje %d suglodinta funkcija' ,i+1));
end
